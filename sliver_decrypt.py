...
def candidate_encoders(session):
    """Collect every plausible encoder for a session.

    The stored encoder comes first (it is usually right), followed by any
    encoder derivable from the URL query-param nonces, and finally every
    encoder we know how to decode. This matters because some Sliver URLs
    carry more than one query value that resolves to a valid encoder id, so
    the extraction step can mislabel a session.
    """
    candidates = []
    ...
    add(session.get('encoder'))

    request_uri = session.get('request_uri', '') or ''
    if '?' in request_uri:
        for part in request_uri.split('?', 1)[1].split('='):
            digits = re.sub('[^0-9]', '', part)
            if digits:
                add(encoders.get(int(digits) % 101))
...
def decode_transport(encoder, raw):
    """Reverse a Sliver transport encoder.

    `raw` is the raw POST-body bytes; the return value is the encrypted blob
    (ChaCha20 nonce + ciphertext). Text encoders operate on an ASCII string,
    while gzip/png carry raw binary and must NOT be ascii-decoded.
    """
    text_encoders = ('hex', 'words', 'gzip-words', 'b64', 'gzip-b64', 'b32', 'b58')
    data = raw.decode('ascii') if encoder in text_encoders else raw

    if encoder == 'hex':
        return unhexlify(data)
    elif encoder == 'words':
        return decode_words(data)
    elif encoder == 'gzip-words':
        return decode_words(data, compressed=True)
    elif encoder == 'b64':
        return decode_b64(data)
    elif encoder == 'gzip-b64':
        return decode_b64(data, compressed=True)
    elif encoder == 'b32':
        return decode_b32(data)
    elif encoder == 'b58':
        return decode_b58(data)
    elif encoder == 'gzip':
...
def decode_http(possible_keys, file_data):
    ...
    for session in sessions:
        ...
        stored_encoder = session.get('encoder')
        body = session.get('body')

        print(f'[+] Processing: {request_uri}')

        # The extraction stores the body as colon-separated hex of the raw
        # request bytes; turn it back into those raw bytes.
        if ':' in body:
        ...
        # ChaCha20-Poly1305 is authenticated, so only the correct
        # (encoder, key) pair decrypts cleanly. Try each candidate encoder
        # and let the auth tag select the right one.
        for encoder in candidate_encoders(session):
            try:
                cipher_text = decode_transport(encoder, raw)
            except Exception:
                # Wrong encoder for this body (e.g. non-ascii for a text
                # encoder, bad base64 padding, ...) -> try the next one.
                continue
            if not cipher_text:
                continue

            for chacha_key in possible_keys:
                # Ensure key is the correct length, only an issue with some
                # regex queries from a memory dump.
                chacha_key = chacha_key[:32]
                try:
                    decrypted = decrypt_chacha(chacha_key, cipher_text)
                except Exception:
        ...
        if decrypted:
            # Lock in the working key so we don't brute force every payload.
            possible_keys = [used_key]
            relabel = '' if used_encoder == stored_encoder else f' (mislabelled as "{stored_encoder}")'
            print(f'  [-] Encoder: {used_encoder}{relabel}')
            print(f'  [-] Session Key: {hexlify(used_key).decode()}')
            try:
                parse_output(decrypted)
            except Exception:
                pass
        else:
            print(f'  [-] Encoder (labelled): {stored_encoder}')
            print('  [!] Session Key: Unable to find a valid key for this session')
